...
from project.DotaApiWrapper import DotaAPIWrapper
...
class DotaAnalysisWrapper():
    def __init__(self):
        self.home = Path(__file__).resolve()
        self.api = self.get_api()

    def get_api(self) -> DotaAPIWrapper:
        config = ConfigParser()
        config.read(Path(self.home.parent.resolve(), "Web.config"))
        config["SECRETS"]["api_key"]
        return DotaAPIWrapper(config["SECRETS"]["api_key"])

    def plot_matchups(self, hero_id: int, window: int=5) -> None:
